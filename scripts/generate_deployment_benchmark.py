...
def main():
    ...
    results = {}
    for name, fn in [("lightgbm", _bench_lightgbm), ("xgboost", _bench_xgboost),
                      ("gnn", _bench_gnn), ("transformer", _bench_transformer),
                      ("rl_dqn", _bench_rl)]:
        print(f"  {name}...")
        results[name] = fn()
    (OUTPUTS / "deployment_benchmark.json").write_text(
