...
def main():
    ...
    result = {
        ...
        "weekday_demand_distribution": [
            {
            "weekday": i, "name": ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"][i],
            "demand": weekday_demand[i], "avg_fare": round(weekday_avg_fare[i], 2)
        }
            for i in range(7)
        ],
        ...
        "fare_statistics": {
            ...
            "min_mean_fare_zone": min(
            [i for i in range(ZONE_COUNT) if zone_count[i] > 0],
            key=lambda i: zone_avg_fare[i]
        ) + 1,
        },
    }
